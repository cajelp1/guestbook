...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="slee3375/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/cajelp1/guestbook.git'
            }
        }
        stage('Build') {
            steps {
    ...
    post { 
        always { 
            emailext (attachLog: true, body: 'jenkins', compressLog: true
                    , recipientProviders: [buildUser()], subject: 'jenkins', to: 'cajelp1@gmail.com')
        ...
        success { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'good'
                , message: "이예림 ${JOB_NAME} (${BUILD_NUMBER}) 빌드가 성공적으로 끝났습니다. Details: (<${BUILD_URL} | here >)")
        }
        failure { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'danger'
                , message: "이예림 ${JOB_NAME} (${BUILD_NUMBER}) 빌드가 실패하였습니다. Details: (<${BUILD_URL} | here >)")
    }
  }
}
